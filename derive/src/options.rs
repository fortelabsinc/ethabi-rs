...
impl ContractOptions {
	pub fn from_attrs(attrs: &[Attribute]) -> Result<Self> {
		...
		let functions = get_function_options(attrs)?.into_iter().fold(HashMap::new(), |mut map, option| {
...
fn get_options(attrs: &[syn::Attribute], name: &str) -> Result<Vec<syn::NestedMeta>> {
	...
	match options {
		...
		_ => Err(ethabi::Error::InvalidName("Unexpected meta item".to_string())),
	}
}
...
fn get_option(options: &[syn::NestedMeta], name: &str) -> Result<String> {
	let item = options
		...
		.ok_or_else(|| ethabi::Error::InvalidName(format!("Expected to find option {}", name)))?;
...
fn str_value_of_meta_item(item: &syn::Meta, name: &str) -> Result<String> {
	...
	Err(ethabi::Error::InvalidName(format!(r#"`{}` must be in the form `#[{}="something"]`"#, name, name)))
}
